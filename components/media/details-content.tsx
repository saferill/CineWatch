...
export const MoviesDetailsContent = ({
  ...
  watchLink?: string
  itemType?: import('@/types/movie-result').ItemType
}) => {
    ...
    <section className="container max-w-(--breakpoint-2xl) pb-10 pt-12 lg:pb-20">
      <div className="flex flex-col gap-8 lg:flex-row">
        ...
        <section className="flex flex-col gap-4">
          <DetailsExtraInfo movie={movie} director={director} trailerId={trailerId} watchLink={watchLink} />
          <DetailsCredits movieCredits={movieCredits} />
        </section>
      </div>
      <Suspense fallback={<SliderHorizontalListLoader />}>
        <List title="Recommended" items={recommendedMovies} itemType={itemType} />
      </Suspense>
      <Suspense fallback={<SliderHorizontalListLoader />}>
        <List title="Similar" items={similarMovies} itemType={itemType} />
      </Suspense>
    </section>
  )
}
