...
export const fetchClient = {
  get: async <T>(
    ...
    const isValidHeaderKey = !!apiConfig.headerKey && apiConfig.headerKey !== 'YOUR_TMDB_READ_ACCESS_TOKEN_HERE';
    const useHeaderAuth = isHeaderAuth && isValidHeaderKey;
    const query = {
      ...params,
    ...
    try {
      const res = await fetch(
        `${apiConfig.baseUrl}${queryString.stringifyUrl({ url, query })}`,
        {
          ...
          ...(nextConfig?.cache ? { cache: nextConfig.cache } : { next: nextConfig || { revalidate: 86400 } }),
        }
      )
